...
Future<void> main() async {
  ...
  stdout.writeln('mirror-local-runner gRPC compile service listening on port $port');
...
class MirrorCompileService extends Service {
  ...
  MirrorCompileService()
    : _artifactSigner = ArtifactSigner(
        ...
        secret: Platform.environment['SIGNED_URL_SECRET'] ?? 'local-dev-secret',
      ) {
  ...
  static String _workspaceRoot() {
    return Platform.environment['MIRROR_WORKSPACE_ROOT'] ?? '/tmp/mirror-local-workspaces';
  }
